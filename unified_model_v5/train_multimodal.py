#!/usr/bin/env python3
"""V5 training. Same recipe as v4 -- sampling distributions, MSE, Adam 1e-3,
cosine decay, 120 epochs x 600 steps, five modalities, six exits, random sizes,
the same 90/10 split -- with three mechanical changes that stop the GPU
starving, and both networks trained from scratch:

  1. RGB read from a pre-decoded uint8 npy cache instead of PNG-decoding
     (43 ms/sample -> 0.6 ms; RGB is 2/7 of sampled steps).
  2. Cast/normalise/resize on the GPU, not the loader thread.
  3. Batches built by worker PROCESSES over a shared-memory queue, sidestepping
     the GIL that capped the thread pool.

Workers are forked before CUDA is initialised and never touch the GPU. Combined
with node-local dataset staging this took 120x600 from 6.08 h to ~11 min on one
A100, at equal quality (verified: per-sample values agree to <=0.5 ULP, end-to-
end loss to 7 significant figures).

NOTE: unlike v4 this saves best-mean-PSNR checkpoints AND validates every
epoch, so `encoder_best_<TAG>.pth` is a true best-epoch checkpoint.
"""
import argparse
import hashlib
import json
...
from multimodal_ae import MAX_DEPTH, TIERS, TIER_TAGS, build
...
def main():
    ...
    ap.add_argument("--tier", choices=tuple(TIERS), default="balanced",
                    help="balanced (u=8, 2 extra convs) or fast (u=8, 1 extra conv)")
    ap.add_argument("--tag", default=None, help="checkpoint suffix; defaults to the tier tag")
